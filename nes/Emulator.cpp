...
namespace sn
{
        ...
        m_cpuCycleDuration(std::chrono::nanoseconds(559))
    {
        if(!m_bus.setReadCallback(PPUSTATUS, [&](void) {return m_ppu.getStatus();}) ||
            ...
            // read ONLY one joypad @ this time
  ...
  extern void _VBlankSync();
    ...
    void Emulator::run(std::string rom_path)
    {
        ...
        m_ppu.reset();
        	...
        	// tmp hack
        	//m_cpuCycleDuration *= 2;
        	//m_elapsedTime = std::chrono::high_resolution_clock::now();
        	int cpt=0;
        	
        
          printf("Start reading pad\n");
          while(true) {

              if (cpt%30==0) pad = console.readPad();
        ...
        #else
            m_emulatorScreen.create(NESVideoWidth, NESVideoHeight, m_screenScale, CLR_WHITE);
        #endif
