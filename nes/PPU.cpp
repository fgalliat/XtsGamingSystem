...
{
	#ifdef XTSCONSOLE
	uint16_t m_pictureBuffer[256*240];
	#endif
    ...
    PPU::PPU(PictureBus& bus, VirtualScreen& screen) :
        ...
        #else
          //,
          //m_pictureBuffer(ScanlineVisibleDots, std::vector<uint16_t>(VisibleScanlines, CLR_PINK))
          //m_pictureBuffer
        #endif
    {}
...
//                     m_screen.setPixel(x, y, sf::Color(colors[m_bus.readPalette(paletteAddr)]));
                    ...
                    #else
                    m_pictureBuffer[(y*256)+x] = (uint16_t)(colors[m_bus.readPalette(paletteAddr)]);
                    #endif
                ...
                if (m_cycle >= ScanlineEndCycle)
                {
                    ...
                    for (int x = 0; x < 256; ++x)
                    {
                        ...
                        for (int y = 0; y < 240; ++y)
                        {
                            ...
                            //#else
                            //m_screen.setPixel(x, y, m_pictureBuffer[(y*256)+x]);
                            //#endif
                        }
                    }
                #else
                  console.getScreen()->drawRGB16((320-256)/2,0, 256, 240, m_pictureBuffer);
                #endif
