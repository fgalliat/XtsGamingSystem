...
Logger_TMPDBUG("  0x01 loadRomCPU()");
	 ...
	 // was just "r" instead of "rb" READ-BINARY
	 // so was exiting before EOF !!!!
   //FILE* romInputStream = fopen( romName, "r" );
   FILE* romInputStream = fopen( romName, "rb" );
...
Logger_TMPDBUG("  0x02 loadRomCPU()");
   ...
   do {
     ...
     int readed = -1;
     if ( (readed = fread( &abyte0, 1, 1, romInputStream )) < 1 ) {
       Logger_debug("The rom file is smaller than it should be and may be corrupt");
       printf("The rom file is smaller than it should be and may be corrupt (%d bytes) (GB_MEM %d bytes long)\n", j1, sizeof(GB_MEM) );
       break;
     }
     ...
     //printf("%d bytes readed ->%d<>%02X< \n", j1, abyte0[0], abyte0[0]);
   } while(true);
 ...
 void GameBoyCPU::setTimer() {
   ...
   TIMER_MAX = (GB_MEM[65287] & 2) <= 0 ? ((int) ((GB_MEM[65287] & 0x01) <= 0 ? 16 : 1024)) : (GB_MEM[65287] & 0x01) <= 0 ? 64 : 256;
 }
 ...
 void GameBoyCPU::handleGBInput()
    ...
    unsigned char GameBoyCPU::getMem(int i) {
        ...
        if(i >= 32768 && i < 40960) {
            //try {
                char c1 = VRAM[(i - 32768) + ((GB_MEM[65359] & 0x01) << 13)];
                return c1;
            //}
            //catch(ArrayIndexOutOfBoundsException arrayindexoutofboundsexception1) {
            //    GB_MEM[65357] = 0x00;
            //}
            //STOP = true;
            //Logger.debug("Attempted array out of bounds access in VRAM at vram page: " + Integer.toString(GB_MEM[65359] & 0x01));
            //char c6 = 0x00;
            //return c6;
        }
    ...
    int GameBoyCPU::getSignedMemN() {
        tempChar = getMem(PC++);
        return tempChar <= 0x7F ? tempChar : tempChar - 256;
    }
    ...
    void GameBoyCPU::setMem(int i, unsigned char c) {
        ...
        if(i == 65351 && GB_MEM[323] < 0x80)
        {
            GB_MEM[65351] = c;
            setBackPalette();
        } else
        if(i == 65352 && GB_MEM[323] < 0x80)
        {
            GB_MEM[65352] = c;
            setForePalette0();
        } else
        if(i == 65353 && GB_MEM[323] < 0x80)
        {
            GB_MEM[65353] = c;
            setForePalette1();
        } else
        if(i == 65385 && GB_MEM[323] >= 0x80)
        {
            GB_MEM[65385] = c;
            int l = (GB_MEM[65384] & 0x38) >> 3;
            int j1 = (GB_MEM[65384] & 6) >> 1;
            VIDEO->setBackColour(j1, l, getGBColour((GB_MEM[65384] & 0x01) <= 0 ? rgb2GBUpper(VIDEO->getBackColour(j1, l)) | c : rgb2GBLower(VIDEO->getBackColour(j1, l)) | c << (GB_MEM[65384] & 0x01) * 8));
        ...
        if(i == 65387 && GB_MEM[323] >= 0x80)
        {
            GB_MEM[65387] = c;
            int i1 = (GB_MEM[65386] & 0x38) >> 3;
            int k1 = (GB_MEM[65386] & 6) >> 1;
            VIDEO->setForeColour(k1, i1, getGBColour((GB_MEM[65386] & 0x01) <= 0 ? rgb2GBUpper(VIDEO->getForeColour(k1, i1)) | c : rgb2GBLower(VIDEO->getForeColour(k1, i1)) | c << (GB_MEM[65386] & 0x01) * 8));
        ...
        if(i >= 32768 && i < 40960)
            VRAM[(i - 32768) + 8192 * (GB_MEM[65359] & 0x01)] = c;
        else
...
//        return (color.getGreen() / 8 << 5) + color.getRed() / 8 & 0xff;
  ...
  // @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    void GameBoyCPU::AND(unsigned char c) {
        A &= c;
        A &= 0xFF;
        F = (unsigned char)((A != 0x00 ? 0 : 0x80) | 0x20);
        AF = combine(A, F);
    }
    ...
    unsigned char GameBoyCPU::RR(unsigned char c) {
        ...
        F = (unsigned char)((tempOpCodeRegister != 0 ? 0 : 0x80) | ((c & 0x01) <= 0 ? 0 : 0x10));
        AF = combine(A, F);
        return getLower(tempOpCodeRegister);
    }
    ...
    unsigned char GameBoyCPU::SWAP(unsigned char c) {
        F = (unsigned char)(c != 0x00 ? 0 : 128);
        AF = combine(A, F);
        
        if (DBUG) printf("(CPU)used SWAP table\n");
...
//printf( "C \n");	
        ...
        if(GB_MEM[65348] >= 0x90 && (GB_MEM[65345] & 3) != 1)
        {
            VIDEO->refresh();
        ...
        if(IME && GB_MEM[65295] > 0x00)
            if((GB_MEM[65295] & 0x01) > 0 && (GB_MEM[65535] & 0x01) > 0)
            {
                HALT = false;
                PUSH(PC);
                PC = 64;
...
void GameBoyCPU::createSwapTable()
    {
        const /*unsigned*/ char tmp_SWAP_TABLE[] = {
            (char)0x00, (char)0x10, ' ', '0', '@', 'P', '`', 'p', (char)0x80, (char)0x90, 
            (char)0xA0, (char)0xB0, (char)0xC0, (char)0xD0, (char)0xE0, (char)0xF0, (char)0x01, (char)0x11, '!', '1', 
            'A', 'Q', 'a', 'q', (char)0x81, (char)0x91, (char)0xA1, (char)0xB1, (char)0xC1, (char)0xD1, 
            (char)0xE1, (char)0xF1, (char)0x02, (char)0x12, '"', '2', 'B', 'R', 'b', 'r', 
            (char)0x82, (char)0x92, (char)0xA2, (char)0xB2, (char)0xC2, (char)0xD2, (char)0xE2, (char)0xF2, (char)0x03, (char)0x13, 
            '#', '3', 'C', 'S', 'c', 's', (char)0x83, (char)0x93, (char)0xA3, (char)0xB3, 
            (char)0xC3, (char)0xD3, (char)0xE3, (char)0xF3, (char)0x04, (char)0x14, '$', '4', 'D', 'T', 
            'd', 't', (char)0x84, (char)0x94, (char)0xA4, (char)0xB4, (char)0xC4, (char)0xD4, (char)0xE4, (char)0xF4, 
            (char)0x05, (char)0x15, '%', '5', 'E', 'U', 'e', 'u', (char)0x85, (char)0x95, 
            (char)0xA5, (char)0xB5, (char)0xC5, (char)0xD5, (char)0xE5, (char)0xF5, (char)0x06, (char)0x16, '&', '6', 
            'F', 'V', 'f', 'v', (char)0x86, (char)0x96, (char)0xA6, (char)0xB6, (char)0xC6, (char)0xD6, 
            (char)0xE6, (char)0xF6, (char)0x07, (char)0x17, (char)0x27, '7', 'G', 'W', 'g', 'w', 
            (char)0x87, (char)0x97, (char)0xA7, (char)0xB7, (char)0xC7, (char)0xD7, (char)0xE7, (char)0xF7, (char)0x62, (char)0x18, 
            '(', '8', 'H', 'X', 'h', 'x', (char)0x88, (char)0x98, (char)0xA8, (char)0xB8, 
            (char)0xC8, (char)0xD8, (char)0xE8, (char)0xF8, (char)0x74, (char)0x19, ')', '9', 'I', 'Y', 
            'i', 'y', (char)0x89, (char)0x99, (char)0xA9, (char)0xB9, (char)0xC9, (char)0xD9, (char)0xE9, (char)0xF9, 
            (char)0x6E, (char)0x1A, '*', ':', 'J', 'Z', 'j', 'z', (char)0x8A, (char)0x9A, 
            (char)0xAA, (char)0xBA, (char)0xCA, (char)0xDA, (char)0xEA, (char)0xFA, (char)0x0B, (char)0x1B, '+', ';', 
            'K', '[', 'k', '{', (char)0x8B, (char)0x9B, (char)0xAB, (char)0xBB, (char)0xCB, (char)0xDB, 
            (char)0xEB, (char)0xFB, (char)0x66, (char)0x1C, ',', '<', 'L', (char)0x5C, 'l', '|', 
            (char)0x8C, (char)0x9C, (char)0xAC, (char)0xBC, (char)0xCC, (char)0xDC, (char)0xEC, (char)0xFC, (char)0x72, (char)0x1D, 
            '-', '=', 'M', ']', 'm', '}', (char)0x8D, (char)0x9D, (char)0xAD, (char)0xBD, 
            (char)0xCD, (char)0xDD, (char)0xED, (char)0xFD, (char)0x0E, (char)0x1E, '.', '>', 'N', '^', 
            'n', '~', (char)0x8E, (char)0x9E, (char)0xAE, (char)0xBE, (char)0xCE, (char)0xDE, (char)0xEE, (char)0xFE, 
            (char)0x0F, (char)0x1F, '/', '?', 'O', '_', 'o', (char)0x7F, (char)0x8F, (char)0x9F, 
            (char)0xAF, (char)0xBF, (char)0xCF, (char)0xDF, (char)0xEF, (char)0xFF
        };
